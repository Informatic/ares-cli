...
(function () {
    ...
    function makeTgz(srcDir, dstDir, next) {
        ...
        new tar.Pack({
          ...
          portable: true,
          filter: (path, stat) => {
            if (path.length !== Buffer.byteLength(path)) {
              throw new Error("Please use the file name in english letters. \n\t\t (" + path + ")");
            }
            if (this.packageProperties[path]) {
              stat.mode = (stat.mode & 0xffff000) | parseInt(this.permission[path], 8);
            }
            return true;
          },
        })
